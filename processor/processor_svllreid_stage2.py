...
def do_train_stage2(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if cfg.MODEL.USE_SSL == True:
            for n_iter, (img, vid, target_cam, target_view,aug1,aug2) in enumerate(train_loader_stage2):
                optimizer.zero_grad()
                ...
                if cfg.MODEL.SIE_VIEW:
                    ...
                    target_view = None
                with amp.autocast(enabled=True):
                    score, feat, image_features,aug_outputs = model(x = img, label = target, cam_label=target_cam, view_label=target_view,USE_SSL=True,aug1=aug1,aug2=aug2)
                    logits = image_features @ text_features.t()
                    #Overall loss of stage2
                    loss = loss_fn(score, feat, target, target_cam, logits,aug_outputs)
